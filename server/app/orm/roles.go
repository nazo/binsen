...
var (
	...
	roleColumnsWithoutDefault = []string{"name", "created_at", "updated_at"}
...
func (o *Role) RolePermissions(mods ...qm.QueryMod) rolePermissionQuery {
	...
	queries.SetFrom(query.Query, "\"role_permissions\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"role_permissions\".*"})
	}
...
func (roleL) LoadRolePermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`role_permissions`), qm.WhereIn(`role_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o RoleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("orm: no Role slice provided for delete all")
	}
