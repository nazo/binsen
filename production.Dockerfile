...
RUN apk --no-cache --update add mercurial musl-dev go go-tools libtool postgresql-dev git bash make openssl readline autoconf automake curl bzip2 gzip tar zip gcc g++ nodejs nodejs-dev nodejs-npm curl-dev bzip2-dev libressl-dev ncurses nginx && \
    mkdir -p /client && \
    mkdir -p /server && \
